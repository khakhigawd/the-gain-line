...
function showOppSugg(val) {
  ...
  var found = tennisData.map(p => p.name).filter(n => n.toLowerCase().includes(v) && n !== currentPlayer).slice(0, 8);
...
html_parts = [
    ...
    '<div class="footer">Layback Analytics | ATP 300 Players | Sackmann 2022-2024 + TennisMyLife 2025-2026 | Generated: ' + generated + '</div>',
    '</div>',
